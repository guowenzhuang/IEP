...
public interface ReplyRepository extends JpaRepository<Reply, Integer>, JpaSpecificationExecutor<Reply> {
	...
	@Modifying
	@Transactional
	//@Query(value = "update Reply set replyNum=replyNum+1 where replyId=:replyId and postId=:postId")
	@Query(value = "UPDATE replytb SET reply_num=reply_num+1 where reply_id=?1 and post_id=?2", nativeQuery = true)
	public void updateReplyNum(Integer parentId, Integer postId);
	
	@Modifying
	@Transactional
	@Query(value = "UPDATE replytb SET reply_num=reply_num+1 where reply_parentid=0 and post_id=?1", nativeQuery = true)
